...
in
{
  extraFiles = {
    ...
    "ftplugin/norg.lua".text = ''
      ...
      local function move_inbox_item_to_today()
        local row = vim.api.nvim_win_get_cursor(0)[1]
      ...
      local function move_task_to_inbox()
        local row = vim.api.nvim_win_get_cursor(0)[1]
        local lines = vim.api.nvim_buf_get_lines(0, 0, -1, false)
      ...
      vim.api.nvim_buf_create_user_command(0, "InboxToToday", move_inbox_item_to_today, {
        desc = "Move the INBOX item under the cursor to today's TODO"
      })

      vim.api.nvim_buf_create_user_command(0, "TaskToInbox", move_task_to_inbox, {
        desc = "Move the task under the cursor back to its week's INBOX"
      })
